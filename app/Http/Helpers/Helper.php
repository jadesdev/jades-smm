...
// referral bonus
function giveReferralBonus($user, $amount)
{
    $refer = User::find($user->ref_id);
    $commission = sys_setting('referral_bonus') * $amount / 100;

    if (sys_setting('is_referral') == 1) {
        if ($refer) {
            $refer->bonus += $commission;
            $refer->save();

            $transaction = $refer->transactions()->create([
                'type' => 'credit',
                'code' => getTrx(),
                'service' => 'referral',
                ...
                'image' => 'referral.png',
                'charge' => 0,
                'new_balance' => $refer->bonus - $commission,
                'old_balance' => $refer->bonus,
                'meta' => [
                    'refer_user' => $user->name,
                    'amount' => $commission,
                ],
                'status' => 'successful',
            ]);
            ...
            // send notification
            // sendUserNotification($refer->id, 'Referral Bonus', $trans->message);
        }
    }
}
