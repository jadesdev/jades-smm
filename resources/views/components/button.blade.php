@if ($isLink())
    <a href="{{ $href }}" {{ $attributes->merge(['class' => $getClasses()]) }}
        ...
        @else
           <a href="{{ $href }}" {{ $attributes->merge(['class' => $getClasses()]) }}> 
        @endif
        @if ($loading) <svg class="{{ $getSpinnerClasses() }} mr-2" fill="none" viewBox="0 0 24 24">
        ...
        @elseif($icon && $iconPosition === 'left')
            <i class="{{ $icon }} {{ $getIconClasses() }} mr-2"></i> @endif
        {{ $slot }}
        @if ($icon && $iconPosition === 'right') <i class="{{ $icon }} {{ $getIconClasses() }} ml-2"></i> @endif </a>
    @else
